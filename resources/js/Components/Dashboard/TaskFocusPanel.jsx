                ...
                {columns.map(column => (
                    <div key={column.id} className="flex flex-col gap-4">
                        <div className="flex items-center gap-3 px-4 py-2">
                            <div className={`w-3 h-3 rounded-full ${column.status === 'in_progress' ? 'bg-teal-500 animate-pulse' : column.status === 'done' ? 'bg-emerald-500' : 'bg-slate-300'}`} />
                            <h3 className="text-[13px] font-extrabold text-slate-500 dark:text-slate-400 tracking-tight">
                                {column.title}
                            </h3>
                            <span className="ml-auto text-[11px] font-bold apple-glass border-none px-2.5 py-1 rounded-full text-slate-500">
                        ...
                        {addingToColumn === column.status && (
                            <motion.div
                                ...
                                className="apple-glass p-5 rounded-[2rem] border-teal-500/30"
                            >
                                <form
                                    onSubmit={(e) => {
                                        e.preventDefault();
                                ...
                                >
                                    <div className="space-y-3">
                                        <input
                                            ...
                                            autoFocus
                                            className="w-full bg-white/5 dark:bg-black/20 border-none rounded-2xl py-3 px-4 text-sm font-bold placeholder:text-slate-400 focus:ring-2 focus:ring-teal-500/20"
                                        />
                                        <div className="flex gap-2">
                                            <input
                                                name="due_date"
                                                type="date"
                                                className="flex-1 bg-white/5 dark:bg-black/20 border-none rounded-xl py-2 px-3 text-xs font-bold text-slate-500 focus:ring-2 focus:ring-teal-500/20"
                                                title="Tenggat Waktu"
                                            />
                                            <input
                                                ...
                                                type="number"
                                                placeholder="Menit"
                                                className="w-24 bg-white/5 dark:bg-black/20 border-none rounded-xl py-2 px-3 text-xs font-bold text-slate-500 focus:ring-2 focus:ring-teal-500/20"
                                    ...
                                    <div className="flex items-center justify-end gap-3">
                                        ...
                                        <button
                                            ...
                                            className="apple-button h-9 py-0 flex items-center bg-teal-500 text-white shadow-lg shadow-teal-500/20 disabled:opacity-50"
                                        >
                                            Simpan
                                        </button>
                                    </div>
                        ...
                        <div className="space-y-4 min-h-[150px]">
                            {getTasksByStatus(column.status).map((task) => (
                                <motion.div
                                    ...
                                    className={`group relative apple-glass rounded-[2rem] border-none shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden ${expandedTaskId === task.id ? 'ring-2 ring-teal-500/40' : ''}`}
                                >
                                    {/* Card Header/Preview */}
                                    <div
                                            ...
                                            >
                                                <div className="p-5 space-y-5">
                                                    ...
                                                    {!task.is_completed && (
                                                        ...
                                                        >
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                <span className="text-[12px] font-extrabold tracking-tight">Mulai Fokus</span>
                                                            </div>
                                                        ...
                                                        {task.subtasks && task.subtasks.length > 0 && (
                                                            <div className="space-y-1">
                                                                {task.subtasks.map((subtask) => (
                                                                    <div key={subtask.id} className="flex items-center gap-3 group/sub p-1 hover:bg-white/5 dark:hover:bg-white/5 rounded-xl transition-all">
                                                                        <button
                                                                            onClick={() => handleToggleSubtask(subtask)}
                                                                            className={`flex-shrink-0 w-5 h-5 rounded-full border-2 transition-all flex items-center justify-center active:scale-90
                                                                                ${subtask.is_completed
                                                                                    ? 'bg-emerald-500 border-emerald-500 text-white'
                                                                                    : 'border-slate-300 dark:border-slate-600 bg-transparent hover:border-teal-500'
                                                                                }`}
                                                                        >
                                                                            {subtask.is_completed && <CheckIcon className="w-3.5 h-3.5 stroke-[3]" />}
                                                                        </button>
                                                                        <span className={`text-[13px] font-semibold tracking-tight transition-all flex-1 ${subtask.is_completed ? 'text-slate-400 line-through' : 'text-slate-700 dark:text-slate-200'}`}>
                                                                            {subtask.title}
                                                                        </span>
                                                                    </div>
                                                                ))}
                                                        ...
                                                        {!task.is_completed && (
                                                            ...
                                                            >
                                                                <input
                                                                    name="subtask"
                                                                    type="text"
                                                                    placeholder="Tambah langkah selanjutnya..."
                                                                    disabled={processingId === `add-sub-${task.id}`}
                                                                    className="w-full bg-white/5 dark:bg-black/20 border-none rounded-2xl py-2.5 pl-4 pr-10 text-xs font-semibold text-slate-700 dark:text-slate-200 placeholder:text-slate-400 focus:ring-2 focus:ring-teal-500/10 shadow-inner overflow-hidden"
                                                                />
                                                                <button
                                                                    type="submit"
                                                    ...
                                                    <div className="space-y-4">
                                                        ...
                                                        <div className="relative">
                                                            <textarea
                                                                ...
                                                                placeholder={auth?.user?.premium_features?.quick_notes ? "Tulis catatan atau ide di sini..." : "Fitur Premium: Simpan catatan penting."}
                                                                className={`w-full bg-white/5 dark:bg-black/20 border-none rounded-[1.5rem] p-4 text-[13px] leading-relaxed text-slate-600 dark:text-slate-300 placeholder:text-slate-400 focus:ring-2 focus:ring-teal-500/10 min-h-[120px] transition-all ${!auth?.user?.premium_features?.quick_notes ? 'cursor-not-allowed opacity-60' : ''}`}
                                                            />
                                                    ...
                                                    <div className="space-y-4">
                                                        <div className="flex items-center justify-between">
                                                            <div className="flex items-center gap-2">
                                                                <h5 className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em]">Launch Link</h5>
                                                                {!auth?.user?.premium_features?.auto_open_url && <LockClosedIcon className="w-2.5 h-2.5 text-amber-500" />}
                                                            </div>
                                                            {task.auto_open_url && auth?.user?.premium_features?.auto_open_url && (
                                                                <a
                                                                    href={task.auto_open_url}
                                                                    target="_blank"
                                                                    rel="noopener noreferrer"
                                                                    className="text-[10px] font-bold text-teal-600 hover:text-teal-700 flex items-center gap-1.5 apple-glass border-none px-2.5 py-1 rounded-full"
                                                                >
                                                        ...
                                                        <div className="relative">
                                                            <input
                                                                type="url"
                                                                disabled={!auth?.user?.premium_features?.auto_open_url}
                                                                ...
                                                                placeholder={auth?.user?.premium_features?.auto_open_url ? "https://..." : "Luncurkan otomatis saat bekerja"}
                                                                className={`w-full bg-white/5 dark:bg-black/20 border-none rounded-xl py-2.5 px-4 text-[13px] font-semibold text-teal-600 dark:text-teal-400 placeholder:text-slate-400 focus:ring-2 focus:ring-teal-500/10 transition-all ${!auth?.user?.premium_features?.auto_open_url ? 'cursor-not-allowed opacity-60' : ''}`}
                                                            />
                                                        </div>
                                                        {task.auto_open_url && !task.is_completed && auth?.user?.premium_features?.auto_open_url && (
                                                            <p className="text-[10px] text-slate-400 font-medium italic">
                                                                * Link ini akan terbuka otomatis saat sesi Fokus dimulai.
                                                            </p>
                                                        )}
                                                    </div>
                                                </div>
